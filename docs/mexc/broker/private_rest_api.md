...
1\. Sign up for an account on the official website and apply to become a broker. You must first apply to become a MEXC broker using MEXC Broker Project Application Form.  
...
| --- | --- | --- |
| Authorization Mode | User authorizes a third-party application to provide client\_secret and obtain the authorization code, which is then used to obtain access token and refresh token. | The application has a server, which can store the application key and interact the key with the MEXCOAuth server. |
...
**API Permission: SPOT\_TRANSFER\_READ**
...
**API Permission: SPOT\_DEPOSIT\_WRITE**
...
**API Permission: SPOT\_DEPOSIT\_READ**
...
*   SPOT\_WITHDRAW\_READ
*   SPOT\_WITHDRAW\_WRITE
*   SPOT\_TRANSFER\_READ
*   SPOT\_TRANSFER\_WRITE
*   SPOT\_DEPOSIT\_READ
*   SPOT\_DEPOSIT\_WRITE

...
length - 16 characters) | | enabled | false | boolean | | Enables/disables the
API key. `true` to enable, `false` to disable | | enabled_features | false |
array | | List of enabled advanced on-key features. Available options:  
...
Block Trade approval feature  
 | | ip_whitelist | false | array | | Whitelist provided IP address on a
selected key |

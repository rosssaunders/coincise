...
`PAXG`  
`BNB`  
`USDE` | The currency symbol | | start_timestamp | true | integer | | The
...
from (milliseconds since the UNIX epoch) | | query | false | string | | The
following keywords can be used to filter the results: `trade`, `maker`, `taker`,
`open`, `close`, `liquidation`, `buy`, `sell`, `withdrawal`, `delivery`,
`settlement`, `deposit`, `transfer`, `option`, `future`, `correction`,
`block_trade`, `swap`. Plus withdrawal or transfer addresses | | count | false |
integer | | Count of transaction log entries returned, default - `100`,
maximum - `250` | | subaccount_id | false | integer | | Id of a subaccount | |

...
This API requires [authentication](auth) with an API Key granted \[View Deposits\] permission.  
If you receive an out\_of\_scope error, please verify the permission settings in the API Key Management page.

currency

string

Currency code to filter deposit history. A filter parameter used to narrow down results by currency code. if not provided, returns the latest deposits across all currencies.

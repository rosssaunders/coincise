...
| --------- | ---- | -------- | ----------- |
| quote_currencies | string | Yes | List of quote currencies to query. To query multiple currencies, provide them as a comma-separated string. [Example] SGD,BTC |

## Request Example

```bash
xxxxxxxxxx1curl --request GET      --url https://region-api.upbit.com/v1/ticker/all      --header 'accept: application/json'
```
...
| trade_timestamp | int64 | The timestamp (in milliseconds) when the ticker was requested. |

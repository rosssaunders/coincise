...
| price | optional | string | Specify price for order, not need for market order.
...
| timeInForce | optional | string | [Time in force](https://www.kucoin.com/docs-new/doc-338146) is a special strategy used during trading. Required for limit orders. |
| postOnly | optional | boolean | passive order labels, this is disabled when the order timing strategy is IOC or FOK if **type** is limit. |
...
| visibleSize | optional | string | When **type** is limit, this is Maximum visible quantity in iceberg orders. |
...
| stopPrice | required | string | The trigger price. |

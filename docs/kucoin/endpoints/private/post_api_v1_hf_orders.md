...
| allowMaxTimeWindow | optional | integer | Order failed after timeout of
specified milliseconds, If clientTimestamp + allowMaxTimeWindow < Gateway
received the message time, this order will fail. | | clientTimestamp | optional
| integer | Equal to KC-API-TIMESTAMP, Need to be defined if allowMaxTimeWindow
is specified. |

...
**Source:**
[/api/v1/margin/order/test](https://www.kucoin.com/docs/rest//api/v1/margin/order/test)
...
| stp | optional | string |
[Self Trade Prevention](https://www.kucoin.com/docs-new/api-5176570) is divided
...
FOK | | hidden | optional | boolean | Hidden or not (not shown in order book) |
| iceberg | optional | boolean | Whether or not only visible portions of orders
are shown in iceberg orders | | visibleSize | optional | string | Maximum
visible quantity in iceberg orders | | cancelAfter | optional | integer | Cancel
after n seconds, the order timing strategy is GTT | | funds | optional | string
| When **type** is market, select one out of two: size or funds | | autoBorrow |
optional | boolean | When Margin Account has inefficient balance, our system
autoborrows inefficient assets and opens positions according to the lowest
market interest rate. | | autoRepay | optional | boolean | AutoPay allows the
return of borrowed assets when you close a position. Our system automatically
triggers the repayment and the maximum repayment amount equals to the
filled-order amount. | | marginModel | optional | string | The type of trading,
including cross (cross mode) and isolated (isolated mode). It is set at cross by
default. |
...
| data.clientOid   | required | string | This return value is invalid                                                                                                  |

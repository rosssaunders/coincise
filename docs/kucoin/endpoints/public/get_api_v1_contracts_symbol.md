...
| data.expireDate | required | integer | Expiration date (milliseconds) Null means it will never expire |
| data.settleDate | required | integer | Settlement date (milliseconds) Null indicates that automatic settlement is not supported |
...
| data.settlementFee | required | number | Settlement fee |
...
| data.fairMethod | required | string | Fair price marking method; the Futures contract is null |
| data.fundingBaseSymbol | required | string | Ticker symbol of the base currency |
| data.fundingQuoteSymbol | required | string | Ticker symbol of the quote currency |
| data.fundingRateSymbol | required | string | Funding rate symbol |

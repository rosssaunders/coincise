...
| data.symbol | required | string | unique code of a symbol, it would not change after renaming |
| data.name | required | string | Name of trading pairs, it would change after renaming |
...
| data.baseMinSize | required | string | The minimum order quantity requried to place an order. |
...
| data.priceLimitRate | required | string | Threshold for price portection |

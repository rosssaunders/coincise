...
Interface description: This endpoint returns the amount changes of a specified
user's account.
...
Interface description: This endpoint returns the detail of a specific order. If
...
Interface description: This interface returns the latest status and details of
the order with the specified user-created order number.
...
The calculated maker rebate value inside ‘filled-fees’ would not be paid
immediately.
  ...
  "data": [
    {
      ...
      "is-ice": false,
      "price": "2.5",
      "created-at": 1747640289427,
      "type": "sell-limit",
      "field-amount": "0",
      "field-cash-amount": "0",
      "field-fees": "0",
      "finished-at": 1747640302667,
      "updated-at": 1747640302667,
      "source": "spot-web",
      "state": "canceled",
      "canceled-at": 1747640302663,
      "canceled-source": 116,
      "canceled-source-desc": "Manual cancelation - Web",
      "role": "none"
    }
  ]
}
...
Interface description: This endpoint returns orders based on a specific
searching criteria. The orders created via API will no longer be queryable after
being cancelled for more than 2 hours.
...
Interface description: This endpoint returns the match results of past and
current filled, or partially filled orders based on specific search criteria.
...
Interface description: Conditional order can be only placed via this endpoint
instead of any endpoint in "Trading" section.
...
• The gap between orderPrice and stopPrice shouldn't exceed the price limit
ratio. For example, a limit buy order's price couldn't be higher than 110% of
market price, this limitation should be also applicable to orderPrice/stopPrice
ratio.

• User has to make sure the clientOrderId's uniqueness. While the conditional
order being triggered, if the clientOrderId is duplicated with another order
(within 24hour) coming from same user, the conditional order will fail
triggering.

• User has to make sure the corresponding account has sufficient fund for
triggering this conditional order, otherwise it would cause conditional order
triggering failure.

• timeInForce enum values: gtc - good till cancel，boc - book or cancel (also
called as post only, or book only), ioc - immediate or cancel, fok - fill or
kill
...
Interface description: The endpoint returns loan interest rates and quota
applied on the user.
...
Interface description: This endpoint repays margin loan with you asset in your
margin account.
...
Interface description: This endpoint is used by the parent user to create sub
users, up to 50 at a time
...
Interface description: Via this endpoint, parent user is able to query sub
user's status by specifying a UID.
...
Interface description: Parent user is able to set asset transfer permission for
a batch of sub users. By default, the asset transfer from sub user’s spot
account to parent user’s spot account is allowed.
...
Interface description: Via this endpoint parent user is able to query account
list of sub user by specifying a UID.
...
Interface description: This endpoint is used by the parent user to modify the
API key of the sub user
...
Interface description: This endpoint is used by the parent user to delete the
API key of the sub user.
...
Interface description: This endpoint returns the aggregated balance from all the
sub-users.

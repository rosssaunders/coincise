...
**Source:**
[open order.](https://docs.backpack.exchange/#tag/Order/operation/get_order)
...
Retrieves an open order from the order book. This only returns the order if it
is resting on the order book (i.e. has not been completely filled, expired, or
cancelled).

One of `orderId` or `clientId` must be specified. If both are specified then the
...
| --------- | -------- | ---------------- | ---------------------------- |
| clientId  | optional | integer <uint32> | Client ID of the order.      |
| orderId   | optional | string           | ID of the order.             |

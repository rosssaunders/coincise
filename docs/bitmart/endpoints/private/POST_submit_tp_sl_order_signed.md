...
**Source:**
[Submit TP/SL Order (SIGNED)](https://developer-pro.bitmart.com/en/futuresv2/)
...
oneway mode  
\-`2`\=buy(reduce only)  
\-`3`\=sell(reduce only) | | size | Int | No | Order size (Number of contracts)
Default the size of position | | trigger_price | String | Yes | Trigger price |
| executive_price | String | Yes | Execution price | | price_type | Int | Yes |
Trigger price type  
\-`1`\=last_price  
\-`2`\=fair_price | | plan_category | Int | No | TP/SL type  
\-`1`\=TP/SL  
\-`2`\=Position TP/SL(default) | | client_order_id | String | No |
Client-defined OrderId(A combination of case-sensitive alphanumerics, all
numbers, or all letters of up to 32 characters) | | category | String | No |

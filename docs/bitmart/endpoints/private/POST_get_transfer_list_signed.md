...
**Source:**
[Get Transfer List (SIGNED)](https://developer-pro.bitmart.com/en/futuresv2/)
...
- If the time range `time_start` and `time_end` are not filled in, all data will
  be displayed by default.
- When filling in the time range, `time_end` must be greater than the value of
  `time_start`.
- If only `time_start` is filled in, query the historical records starting from
  the timestamp.
- If only `time_end` is filled in, query the historical records starting from
  this timestamp.
...
\-`spot_to_contract`  
\-`contract_to_spot` | | state | String | Result  
\-`PROCESSING`\=Waiting to execute  
\-`FINISHED`\=Successful transfer  
\-`FAILED`\=Transfer failed | | timestamp | Long | Transfer creation time in
milliseconds, e.g. 1638631674326 |

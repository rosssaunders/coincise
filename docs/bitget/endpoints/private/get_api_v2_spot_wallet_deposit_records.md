...
Token name | | type | String | Type  
Fixed value: `deposit` | | size | String | Quantity | | status | String |
Withdrawal status  
`pending`: pending confirmation  
`fail`: failed  
`success`: successed | | fromAddress | String | Deposit Initiators  
...
if `dest` is `internal_transfer`, please ignore this parameter | | dest | String
| Deposit Type  
`on_chain`: the on chain deposit  
`internal_transfer`: internal deposit | | cTime | String | Creation time, ms | |

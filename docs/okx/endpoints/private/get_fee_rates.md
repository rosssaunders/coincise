...
e.g. `BTC-USD`  
Applicable to `FUTURES`/`SWAP`/`OPTION` | | ruleType | String | No | Trading
rule types  
...
`normal`: normal trading  
`pre_market`: pre-market trading | | ts | String | Data return time, Unix
timestamp format in milliseconds, e.g. `1597026383085` | | category | String |
Currency category. Note: this parameter is already deprecated | | fiat | Array
of objects | Details of fiat fee rate | | \> ccy | String | Fiat currency. | |
\> taker | String | Taker fee rate | | \> maker | String | Maker fee rate |

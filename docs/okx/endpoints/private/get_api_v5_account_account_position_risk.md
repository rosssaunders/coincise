...
`cross`  
`isolated` | | \> posId | String | Position ID | | \> instId | String |
Instrument ID, e.g. `BTC-USDT-SWAP` | | \> pos | String | Quantity of positions
`contract`. In the isolated margin mode, when doing manual transfers, a position
with pos of `0` will be generated after the deposit is transferred | | \>
baseBal | String | ~Base currency balance, only applicable to `MARGIN`（Quick
Margin Mode）~(Deprecated) | | \> quoteBal | String | ~Quote currency balance,
only applicable to `MARGIN`（Quick Margin Mode）~(Deprecated) | | \> posSide |
String | Position side  
`long`  
`short`  
`net` (`FUTURES`/`SWAP`/`OPTION`: positive `pos` means long position and
negative `pos` means short position. `MARGIN`: `posCcy` being base currency
means long position, `posCcy` being quote currency means short position.) | | \>
posCcy | String | Position currency, only applicable to `MARGIN` positions. | |
\> ccy | String | Currency used for margin | | \> notionalCcy | String |
Notional value of positions in `coin` | | \> notionalUsd | String | Notional

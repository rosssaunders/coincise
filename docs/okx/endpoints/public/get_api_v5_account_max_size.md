...
Source:
[https://www.okx.com/docs-v5/en/#trading-account-rest-api-get-maximum-order-quantity](https://www.okx.com/docs-v5/en/#trading-account-rest-api-get-maximum-order-quantity)
...
`isolated`  
`cash`  
`spot_isolated`: only applicable to `Futures mode`. | | ccy | String |
Conditional | Currency used for margin  
...
Only applicable to `MARGIN/FUTURES/SWAP` | | tradeQuoteCcy | String | No | The
...
The cross-margin order under `Futures mode` mode, quantity of coins is based on
base currency.  
`FUTURES`/`SWAP`/`OPTIONS`: The maximum quantity of contracts that you can buy |
| maxSell | String | `SPOT`/`MARGIN`: The maximum quantity in quote currency
that you can sell  
The cross-margin order under `Futures mode` mode, quantity of coins is based on
base currency.  

...
async function convertSupportArticleToMarkdown(url) {
...
async function convertDocsToMarkdown() {
    // Initialize Turndown with GFM
    const turndownService = new TurndownService({
        codeBlockStyle: 'fenced',
        fence: '```'
    ...
    await page.goto(`file:${path.join(__dirname, 'tools/extraction/deribit/docs/docs.deribit.com/index.html')}`);
...
async function main() {
    try {
        ...
        const mainDocsMarkdown = await convertDocsToMarkdown();
        ...
        for (const url of urls) {
            ...
            const articleMarkdown = await convertSupportArticleToMarkdown(url);
            if (articleMarkdown) {
        ...
        await fs.writeFile('deribit-api-documentation.md', combinedMarkdown);
        console.log('Documentation conversion complete! Output file: deribit-api-documentation.md');

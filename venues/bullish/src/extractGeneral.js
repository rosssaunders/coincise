...
const extractNetworkConnectivity = spec => {
  ...
  const connectivityMatch = description.match(
    /# Connectivity Options\s+([\s\S]*?)(?=\n#|\n\n#|$)/
  )
...
const extractAuthentication = spec => {
  ...
  const description = spec.info.description || ""
  const authMatch = description.match(
...
const extractRateLimits = spec => {
  ...
  const description = spec.info.description || ""
  const rateLimitMatch = description.match(
    /# Rate Limits?\s+([\s\S]*?)(?=\n#|\n\n#|$)/
  )
  if (rateLimitMatch) {
    ...
    if (description.includes("rate limit")) {
      ...
      markdown +=
        "Please refer to the API response headers for rate limit information.\n"
    }
  }
...
const extractErrorCodes = spec => {
  ...
  if (description.includes("Error & Rejection Codes")) {
    ...
    markdown +=
      "For comprehensive error and rejection codes, see the [Error & Rejection Codes](https://github.com/bullish-exchange/api-docs/wiki/Error-&-Rejection-Codes) documentation.\n"
  }
...
const extractChangelog = spec => {
  ...
  if (changelogMatch) {
    ...
    markdown +=
      "Please check the Bullish documentation for the latest changes and updates.\n"
  }
...
const extractGeneralInfo = spec => {
  ...
  const linksMatch = description.match(
    /# Additional Links\s+([\s\S]*?)(?=\n# [A-Z]|$)/
  )
  if (linksMatch) {

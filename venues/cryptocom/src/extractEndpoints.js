  ...
  const endpoints = await page.evaluate(() => {
    ...
    for (const h2 of h2Elements) {
      ...
      if (h2Text.includes("public/") || h2Text.includes("private/")) {
        ...
        if (contentLower.includes("method: get") || contentLower.includes('"method":"get"')) {
          method = "GET";
        } else if (contentLower.includes("method: put") || contentLower.includes('"method":"put"')) {
          method = "PUT";
        } else if (contentLower.includes("method: delete") || contentLower.includes('"method":"delete"')) {
          method = "DELETE";
        }

        // Build source URL with anchor
        const anchorId = h2Text.toLowerCase().replace(/\//g, "-").replace(/\./g, "-");

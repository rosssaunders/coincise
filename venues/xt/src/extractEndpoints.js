...
const isPrivateEndpoint = (content) => {
  ...
  const hasAuthHeaders = hasValidateSignature || hasValidateTimestamp || hasValidateAppkey;
  
  // Check for rate limit patterns that indicate API key is required
  const hasApiKeyRateLimit = content.includes('/apikey') || content.includes('/apiKey');
  
  // Also check for explicit authentication requirement mentions
  const hasAuthMention = content.toLowerCase().includes('authentication required') ||
...
const extractEndpoints = async (page, turndownService) => {
  ...
  await page.goto(`${BASE_URL}/Access%20Description/BasicInformationOfTheInterface`, {

...
const extractNetworkConnectivity = async (page, turndownService) => {
  ...
  const html = await page.evaluate(() => {
    ...
    for (const element of sections) {
      if (element.tagName === 'H1' && element.textContent.includes('Introduction')) {
        capturing = true;
        content.appendChild(element.cloneNode(true));
        continue;
      }

      if (capturing && element.tagName === 'H1' && !element.textContent.includes('Introduction')) {
        break;
      }
...
const extractAuthentication = async (page, turndownService) => {
  ...
  const html = await page.evaluate(() => {
    ...
    const headings = Array.from(document.querySelectorAll('h1, h2, h3, h4, p, ul, ol, pre, code'));
    let capturing = false;
...
const extractChangelog = async (page, turndownService) => {
  ...
  const html = await page.evaluate(() => {
    ...
    for (const element of elements) {
      ...
      if (capturing && (element.tagName === 'H2' || element.tagName === 'H3' ||
          element.tagName === 'P' || element.tagName === 'UL' || element.tagName === 'OL')) {
        content.appendChild(element.cloneNode(true));
      }
    }
...
const extractRateLimits = async (page, turndownService) => {
  ...
  const html = await page.evaluate(() => {
    ...
    const elements = Array.from(document.querySelectorAll('h1, h2, h3, p, table, ul, ol'));
    let capturing = false;
...
const extractErrorCodes = async (page, turndownService) => {
  ...
  const html = await page.evaluate(() => {
    ...
    const elements = Array.from(document.querySelectorAll('h1, h2, h3, p, table, ul, ol, pre'));
    let capturing = false;
    ...
    for (const element of elements) {
      ...
      if ((element.tagName === 'H1' || element.tagName === 'H2') &&
          (text.includes('error') && (text.includes('code') || text.includes('response')))) {
        capturing = true;
        content.appendChild(element.cloneNode(true));
        continue;
      }
...
const extractResponseFormats = async (page, turndownService) => {
  ...
  const html = await page.evaluate(() => {
    ...
    content.innerHTML = '<h1>Response Formats</h1><p>All responses from the Backpack Exchange API follow standard JSON format.</p>';

...
const extractEndpoints = async (page, turndownService) => {
  ...
  const endpoints = await page.evaluate(async () => {
      ...
      tables.forEach(table => {
        ...
        rows.forEach(row => {
          ...
          if (cells.length === 2) {
            ...
            const required = firstCell.textContent.toLowerCase().includes('required');
            const typeElements = secondCell.querySelectorAll('.sc-gFAWRd');

            // Look for description in multiple possible locations
            const descriptionElem = secondCell.querySelector('.sc-eeDRCY, .sc-eBMEME, p, [class*="description"]');
            ...
            if (descriptionElem) {
              ...
              for (let i = 0; i < lines.length; i++) {
                ...
                if (!line.match(/^(string|number|integer|boolean|array|object|Enum:)$/i) &&
                    !line.startsWith('"') &&
                    line.length > 10) {
                  descText = line;
                  break;
            ...
            if (propertyName) {
              ...
              const typeText = Array.from(typeElements).map(el => el.textContent.trim()).join(' ');
    ...
    const expandResponseButtons = () => {
      ...
      const responseButtons = buttons.filter(btn => {
        ...
        return /^\s*\d{3}\s/.test(text) && // Starts with 3-digit code
    ...
    operations.forEach((operation, index) => {
      try {
        ...
        const originalOperationId = operationId; // Save for debug

        // Try to find method and path
        const methodElement = operation.querySelector('[data-role="method"], .http-verb, .method');
        const pathElement = operation.querySelector('[data-role="path"], .path, .endpoint-path');
        ...
        if (!operationId || operationId.trim() === '') {
          ...
          operationId = endpointPath.replace(/\./g, '').replace(/\s+/g, '_').toLowerCase();
        }
        ...
        const hasResponseSchema = content.includes('Response Schema') || content.includes('RESPONSE SCHEMA');
        ...
        while (nextSibling && siblingCount < 3) {
          ...
          if (nextSibling.textContent.toLowerCase().includes('response') ||
        ...
        const finalSourceUrl = (operationId && operationId.trim().length > 0) ?
    ...
    if (results.public.length === 0 && results.private.length === 0) {
      console.log('⚠️ Using fallback extraction method - primary method found no endpoints');
  ...
  // Log debug info
  if (endpoints._debug) {
    console.log('📝 Debug first endpoint:', JSON.stringify(endpoints._debug, null, 2));
  }
  if (endpoints._debugSteps) {
    console.log('📝 Debug extraction steps:', JSON.stringify(endpoints._debugSteps, null, 2));
  }

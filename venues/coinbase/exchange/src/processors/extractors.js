...
export const extractArticleContent = async (page, options = {}) => {
  return await page.evaluate(returnHtml => {
    ...
    if (returnHtml) {
      ...
      const bodyParamsHeadings = Array.from(clone.querySelectorAll('h3')).filter(
        h => h.textContent.trim() === 'Body params'
      )
...
export const extractRequestParams = async page => {
  return await page.evaluate(() => {
    ...
    const bodyParamsHeadings = Array.from(document.querySelectorAll('h3')).filter(
      h => h.textContent.trim() === 'Body params'
    )
...
export const extractModalContent = async page => {
  return await page.evaluate(() => {
    ...
    if (muiPaper) {
      ...
      const schemaType =
        muiPaper.querySelector('.MuiCardHeader-content .MuiTypography-root span')?.textContent || ''
      ...
      listItems.forEach(item => {
        ...
        if (descriptionElement) {
          ...
          const headerText =
            descriptionElement.querySelector('.paramHeader_i_6k')?.textContent || ''

...
const main = async () => {
  ...
  try {
    ...
    await page.goto("https://docs.cdp.coinbase.com/api-reference/international-exchange-api", {
    ...
    const refPageData = await page.evaluate(() => {
      ...
      return {
        ...
        allLinks: links.map(l => ({ text: l.textContent.trim(), href: l.href }))
    ...
    await page.goto("https://docs.cdp.coinbase.com/api-reference/international-exchange-api/rest-api/introduction", {
      waitUntil: "networkidle2",
      timeout: 90000
    });
    ...
    const introData = await page.evaluate(() => {
      ...
      return {
        ...
        links: links.map(l => ({ text: l.textContent.trim(), href: l.href }))
          .filter(l => l.text && (l.href.includes('portfolio') || l.href.includes('order') || l.href.includes('instrument')))
          .slice(0, 20)
      };
    });
    ...
    const docsData = await page.evaluate(() => {
      ...
      const portfolioLinks = links
        ...
        .filter(l => l.href.includes('portfolio') || l.href.includes('order') || l.href.includes('instrument'));
    ...
    const navData = await page.evaluate(() => {
      const navItems = Array.from(document.querySelectorAll('nav a, [role="navigation"] a, aside a'));
      return navItems
        .map(l => ({ text: l.textContent.trim(), href: l.href }))
    ...
    const specCheck = await page.evaluate(() => {
      ...
      return {
        hasOpenAPI: pageText.includes('openapi') || pageText.includes('swagger'),
        hasSpec: pageText.includes('specification') || pageText.includes('spec.json'),
        apiBaseUrl: pageText.match(/https:\/\/api\.international\.coinbase\.com[^\s]*/)?.[0] || null
      };
    });

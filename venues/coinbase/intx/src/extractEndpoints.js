...
const BASE_URL =
  "https://docs.cdp.coinbase.com/api-reference/international-exchange-api/reference"
    ...
    const cleanupCoinbaseParams = () => {
      ...
      headers.forEach(header => {
        ...
        paramDivs.forEach(paramDiv => {
          ...
          const descElement = currentElement.querySelector(
            'p, [class*="description"]'
          )
          const requiredElement = currentElement.querySelector(
            '[class*="required"]'
          )
...
const main = async () => {
  ...
  console.log(
    `Extracting ${Math.min(MAX_ENDPOINTS, KNOWN_ENDPOINTS.length)} of ${KNOWN_ENDPOINTS.length} endpoints`
  )

...
const SPEC_URL = "https://docs.cdp.coinbase.com/api-reference/international-exchange-api/rest-api/intx-spec.yaml";
...
const isPublicEndpoint = (pathStr, method, endpoint) => {
  ...
  const hasPublicTag = endpoint.tags && endpoint.tags.some(tag => publicTags.includes(tag));
  
  // Check the path - public endpoints typically don't include portfolio or require auth
  const isPublicPath = !pathStr.includes('portfolio') && 

...
const splitIntoEndpoints = (html, sourceUrl, isPrivate) => {
  ...
  const headingLevel = headingElements.length > 0 ? headingElements[0].tagName : 'H3';
  ...
  for (let i = 0; i < headingElements.length; i++) {
    ...
    if (endpointContent.length > 0) {
      ...
      if (methodPath) {
        ...
        console.log(`  Skipping endpoint "${title}" - no HTTP method/path found`);
      }
    }
  }
 ...
 */
const extractEndpointsFromPage = async (page, endpoint, isPrivate, turndownService) => {

...
const log = (level, color, icon, message, ...args) => {
  console.log(`[${getTimestamp()}] ${color}${icon} [${level}]${RESET} ${message}`, ...args)
}

const errorLog = (message, ...args) => {
  console.error(`[${getTimestamp()}] ${RED}${ERROR_ICON} [ERROR]${RESET} ${message}`, ...args)
}
...
export const logger = {
  ...
  success: (message, ...args) => log('SUCCESS', MAGENTA, SUCCESS_ICON, message, ...args),
}

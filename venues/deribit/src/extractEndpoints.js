...
const endpointToFilename = (endpointPath) => {
  ...
  const postPatterns = ['buy', 'sell', 'edit', 'cancel', 'close', 'set', 'enable', 'disable', 'subscribe', 'unsubscribe'];
...
const extractEndpoints = async (page, turndownService) => {
  ...
  const endpoints = await page.evaluate(() => {
    ...
    h2Elements.forEach(h2 => {
      ...
      if (endpointText.startsWith('/public/') || endpointText.startsWith('/private/')) {

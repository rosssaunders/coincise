...
function processAndGenerateMarkdown(sectionsData, configObj) {
  ...
  const filteredSections = filterSectionsByConfig(
    sectionsData,
    configObj
  );
  
  // Generate markdown with the filtered sections
  return formatEndpointsMarkdown(
    filteredSections,
    configObj.title
  );
}
...
async function convertHtmlToMarkdown(apiType) {
  ...
  try {
    ...
    const configFiles = fs.readdirSync(path.join(__dirname, config.configDir))
    ...
    for (const configFile of configFiles) {
      ...
      const configObj = JSON.parse(fs.readFileSync(path.join(__dirname, config.configDir, configFile), 'utf8'));

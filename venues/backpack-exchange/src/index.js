...
turndownService.remove(['style', 'script', 'noscript', 'meta', 'link', 'nav', 'header', 'footer'])

// Add custom rule to ignore CSS and other unwanted content
turndownService.addRule('removeStyles', {
  filter: ['style', 'script', 'noscript', 'head', 'meta', 'link', 'nav', 'header', 'footer'],
  replacement: () => ''
})
  ...
  try {
    ...
    const openAPIContent = await page.evaluate(() => {
      ...
      for (const pre of preElements) {
        ...
        if (text.startsWith('{') && (text.includes('openapi') || text.includes('swagger'))) {
          return [{
            tagName: 'pre',
            textContent: text
          }]
    ...
    if (openAPIContent) {
      console.log('Found potential OpenAPI content:', openAPIContent.length, 'elements')
      ...
      if (specContent) {
        ...
        console.log('✅ API documentation saved to docs/backpack/api_documentation.md')
    ...
    console.log('OpenAPI spec not found directly, extracting general documentation...')
    
    // Extract content and clean it in the browser first
    const mainContent = await page.evaluate(() => {
      // Remove unwanted elements from the entire document first
      const unwantedElements = document.querySelectorAll('style, script, noscript, link, meta, nav, header, footer')
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.length > 0) {
        ...
        if (line.length < 50 && i < lines.length - 1 && !line.includes('.') && !line.includes(',')) {
          cleanMarkdown += `## ${line}\n\n`
        } else {
          cleanMarkdown += `${line}\n\n`
        }
    ...
    console.log('✅ General documentation saved to docs/backpack/general_documentation.md')

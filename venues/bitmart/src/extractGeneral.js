...
const extractSectionByTitle = async (page, sectionTitle) => {
  ...
  const html = await page.evaluate((title) => {
    ...
    while (currentElement) {
      ...
      if (currentElement.tagName && currentElement.tagName.toLowerCase() === 'h1') {
        break;
      }
...
const extractNetworkConnectivity = async (page, turndownService) => {
  ...
  const html = await page.evaluate(() => {
    ...
    for (const element of allElements) {
      ...
      if (text && (text.includes('Base URL') || text.includes('Request URL') || text.includes('REST API'))) {
        if (element.tagName === 'H2' || element.tagName === 'H3') {
          ...
          let count = 0;
          while (sibling && count < 5 && !['H1', 'H2'].includes(sibling.tagName)) {
            content.appendChild(sibling.cloneNode(true));
            sibling = sibling.nextElementSibling;
            count++;
          }
...
const extractResponseFormats = async (page, turndownService) => {
  ...
  const html = await page.evaluate(() => {
    ...
    for (const element of sections) {
      ...
      if (text.includes('response') && (text.includes('format') || text.includes('structure'))) {
        capturing = true;
        content.appendChild(element.cloneNode(true));
      } else if (capturing) {
...
const main = async () => {
  ...
  try {
    ...
    const authentication = await extractAuthentication(spotPage, turndownService);
    ...
    const networkConnectivity = await extractNetworkConnectivity(spotPage, turndownService);
    ...
    const responseFormats = await extractResponseFormats(spotPage, turndownService);

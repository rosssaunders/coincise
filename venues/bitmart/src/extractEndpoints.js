...
const extractEndpoints = async (page, sourceUrl, apiType) => {
  ...
  const endpoints = await page.evaluate((srcUrl, type) => {
    ...
    for (const heading of headings) {
      ...
      if (!headingText || headingText.toLowerCase().includes('overview') || 
      ...
      const urlMatch = contentHtml.match(/(?:GET|POST|DELETE|PUT)\s+(https?:\/\/[^\s<]+)/i);
      if (urlMatch) {
      ...
      if (!endpointPath) {
        endpointPath = '/' + headingText.toLowerCase()
          .replace(/[^a-z0-9]+/g, '_')
          .replace(/^_|_$/g, '');
      }
      ...
      const hasKeyedAuth = contentHtml.toUpperCase().includes('KEYED') ||
                           contentHtml.includes('X-BM-KEY') ||
                           contentHtml.includes('X-BM-SIGN') ||
                           contentHtml.includes('X-BM-TIMESTAMP');
      
      const hasSignedAuth = contentHtml.toUpperCase().includes('SIGNED') ||
...
const main = async () => {
  ...
  try {
    ...
    const spotEndpoints = await extractEndpoints(spotPage, BASE_URL_SPOT, 'Spot');
    ...
    const futuresEndpoints = await extractEndpoints(futuresPage, BASE_URL_FUTURES, 'Futures');
    ...
    console.log(`  Spot - Public: ${spotEndpoints.public.length}, Private: ${spotEndpoints.private.length}`);
    console.log(`  Futures - Public: ${futuresEndpoints.public.length}, Private: ${futuresEndpoints.private.length}`);
  } catch (error) {

...
async function getPageHTML(pageURL, browser) {
  ...
  const html = await page.evaluate(async () => {
    async function processH2(sectionElement) {
      console.log('Processing', sectionElement.querySelector('h2').textContent.trim())

      // Create DIV to hold the details we want to keep
      const detailDiv = document.createElement('div')
      detailDiv.className = 'convert-to-markdown'
      sectionElement.parentElement.insertBefore(detailDiv, sectionElement.nextSibling)
      ...
      sectionElement.querySelectorAll('p.item-code-content').forEach(p => {
        if (
          p &&
          (p.textContent.trim().startsWith('POST') || p.textContent.trim().startsWith('GET'))
        ) {
      ...
      sectionElement.querySelectorAll('div').forEach(div => {
        if (div) {
          const buttonChild = Array.from(div.children).find(child => child.tagName === 'BUTTON')
          if (buttonChild) {
          ...
          const aLink = Array.from(div.children).find(child => child.tagName === 'A')
          if (aLink) {
      ...
      sectionElement.querySelectorAll('label').forEach(label => {
        if (label && label.textContent.trim() === 'Content-Type:') {
          label.nextElementSibling.querySelectorAll('button').forEach(button => {
          ...
          contentTypeText.textContent =
            label.textContent.trim() + label.nextElementSibling.textContent.trim()
      ...
      const contentDiv = sectionElement.querySelector('.el-collapse-item__content')
      if (contentDiv) {
      ...
      await processTableSection('#tab-request', '#pane-request', 'Request Parameters')
      await processTableSection('#tab-response', '#pane-response', 'Response Parameters')
      ...
      await processTableSection('#tab-response1', '#pane-response1', 'Order Parameters')

      const errorLabel = Array.from(sectionElement.querySelectorAll('label span')).find(span =>
        span.textContent.includes('errors')
      )
      ...
      if (errorLabel) {
        ...
        if (label) {
          ...
          for (const item of errorItems) {
            ...
            const descriptionElement = item.querySelector('.el-collapse-item__content')

            if (descriptionElement) {
              const errorData = JSON.parse(descriptionElement.textContent.trim())
      ...
      sectionElement.querySelectorAll('[role="radiogroup"]').forEach(el => el.remove())

      sectionElement.querySelectorAll('div.el-tabs__header.is-top').forEach(el => el.remove())

      sectionElement.querySelectorAll('div.item-demo-content').forEach(el => el.remove())
    ...
    const sectionContainers = Array.from(body.querySelectorAll('.section-container'))

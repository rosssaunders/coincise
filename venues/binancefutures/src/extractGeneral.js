 ...
 */
const extractGeneralForType = async (
  page,
  turndownService,
  type,
  outputDir
  ...
  if (generalInfo) {
    ...
    await writeFile(
      path.join(outputDir, "network_connectivity.md"),
      networkConnectivity
    )
  ...
  try {
    ...
    if (commonDefs) {
      ...
      await writeFile(
        path.join(outputDir, "response_formats.md"),
        responseFormats
      )
    }
  } catch (e) {
...
const main = async () => {
  console.log(
    "Starting general documentation extraction for Binance Futures..."
  )

    ...
    const html = await page.evaluate(() => {
      ...
      for (const selector of selectors) {
        ...
        if (element && element.innerHTML.trim()) {
          ...
          const unwanted = clone.querySelectorAll('button, .copy-button, [class*="copy"]');
...
const extractNetworkConnectivity = async (page, turndownService) => {
  ...
  const urls = [
    `${BASE_URL}/common/intro`,
    `${BASE_URL}/common/domain`
  ];
...
const extractRateLimits = async (page, turndownService) => {
  ...
  return retryWithBackoff(async () => {
    ...
    const html = await page.evaluate(() => {
      ...
      const headings = Array.from(document.querySelectorAll("h1, h2, h3, p, ul, li"));
      let capturing = false;
...
const extractResponseFormats = async (page, turndownService) => {
  ...
  return retryWithBackoff(async () => {
    ...
    const html = await page.evaluate(() => {
      ...
      const elements = Array.from(document.querySelectorAll("h1, h2, h3, p, pre, code"));
      
      for (const element of elements) {
        if (element.textContent.toLowerCase().includes("response") ||

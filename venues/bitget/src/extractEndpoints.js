...
const isPublicEndpoint = (html, title) => {
  ...
  if (htmlLower.includes("authentication required") ||
...
const extractEndpoint = async (page, url, turndownService) => {
  return retryWithBackoff(async () => {
    ...
    let cleanHtml = result.html.replace(/(Copy Success|Copy to Clipboard)/gi, "");
  ...
  try {
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      if (endpoint) {
        ...
        console.log(`  ✓ Wrote: ${endpoint.isPublic ? "public" : "private"}/${filename}`);

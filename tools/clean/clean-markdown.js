...
async function cleanMarkdown(inputPath, outputPath) {
  try {
    ...
    console.log(`Successfully cleaned markdown from ${inputPath} to ${outputPath}`);
  } catch (error) {
    if (error.code === 'MODULE_NOT_FOUND') {
      console.error('Error: Required modules not found. Please run "npm install" in this directory.');
    } else {
...
if (!inputPath) {
  console.error('Please provide an input file path: node clean-markdown.js <input-file> [output-file]');
  process.exit(1);
}

...
This directory contains scripts for managing dependencies across all JavaScript
projects in the coincise repository.

## Scripts

### `update-dependencies-robust.sh`

Updates all dependencies across all JavaScript projects to their latest versions
...
- Runs `npm update` to update within semver ranges (e.g., `^7.1.2` stays within
  `^7.x.x`)
...
Bumps all dependencies to their absolute latest versions across all JavaScript
projects (like dependi does).
...
- For `bump-dependencies.sh`: `npm-check-updates` will be installed
